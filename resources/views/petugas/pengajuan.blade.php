...
@section('content')
            ...
            <tbody>
                @forelse($pending as $loan)
                    <tr>
                        ...
                        <td>{{ $loan->user->nik ?? '-' }}</td>
                        <td><span class="status-badge disetujui">{{ $loan->duration ?? 7 }} hari</span></td>
                        <td>{{ $loan->qty ?? 1 }}</td>
                        <td>
                            <div class="aksi-group">
                                <button class="btn-tolak" data-id="{{ $loan->id }}" data-buku="{{ $loan->book->title ?? '' }}"
                                    data-tgl="{{ \Carbon\Carbon::parse($loan->loan_date)->format('d/m/Y') }}">
                                    Tolak
                                </button>
                                <button type="button" class="btn-pinjamkan" data-id="{{ $loan->id }}">
                                    Pinjamkan
                                </button>
                            </div>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="empty-row">Tidak ada pengajuan pending.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
            ...
            <tbody>
                @forelse($confirmed as $loan)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('petugas.pengajuan.detail', $loan) }}" class="btn-detail-icon"
                                title="Detail">&#128441;</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="empty-row">Belum ada konfirmasi.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="modal-overlay" id="modalTolak">
        <div class="modal-box" style="max-width:480px;">
            ...
            <textarea id="rejectionReason" rows="6"
                style="width:100%; padding:12px; border:1.5px solid #d1d5db; border-radius:10px; font-size:13px; font-family:inherit; resize:vertical; outline:none; box-sizing:border-box;"></textarea>
