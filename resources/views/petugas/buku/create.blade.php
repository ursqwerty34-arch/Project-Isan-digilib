...
<div class="buku-form-card">
    <form method="POST" action="{{ route('petugas.buku.store') }}" enctype="multipart/form-data" id="formBuku">
        @csrf
        <div class="buku-form-layout">
            ...
            <div class="buku-fields-side">
                <div class="buku-field">
                    ...
                    <input type="text" name="kode_buku" placeholder="Kode buku" value="{{ old('kode_buku') }}" required>
                    @error('kode_buku')<span class="field-error">{{ $message }}</span>@enderror
                </div>
                <div class="buku-field">
                    <label>Penulis*</label>
                    <input type="text" name="author" placeholder="Penulis" value="{{ old('author') }}" required>
                    @error('author')<span class="field-error">{{ $message }}</span>@enderror
                </div>
                <div class="buku-field">
                    <label>Tahun terbit*</label>
                    <div class="input-date-wrap">
                        <input type="date" name="year" value="{{ old('year') }}" required>
                    </div>
                    @error('year')<span class="field-error">{{ $message }}</span>@enderror
                </div>
                <div class="buku-field">
                    <label>Judul Buku*</label>
                    <input type="text" name="title" placeholder="Judul Buku" value="{{ old('title') }}" required>
                    @error('title')<span class="field-error">{{ $message }}</span>@enderror
                </div>
                <div class="buku-field">
                    <label>Penerbit</label>
                    <input type="text" name="publisher" placeholder="Penerbit" value="{{ old('publisher') }}">
                </div>
                <div class="buku-field">
...
dropzone.addEventListener('dragover', e => { e.preventDefault(); dropzone.style.borderColor = '#2f5d34'; });
dropzone.addEventListener('dragleave', () => { dropzone.style.borderColor = '#a5d6a7'; });
